...
async function runTest() {
    const secret = "This is my hidden message";
    const passphrase = "mySecretPassword123";

    const encrypted = encryptMessage(secret, passphrase);
    console.log("Encrypted payload (bytes):", encrypted.length);

    await embedInImage('cover.png', encrypted, 'stego.png');

    const extractedPayload = await extractFromImage('stego.png');
